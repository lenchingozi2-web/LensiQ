    ...
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3">
          <p className="text-xs font-bold text-slate-400">{[result.subject, result.division, result.topic].filter(Boolean).join(' · ') || 'LenxiQ question bank'}</p>
      ...
      <div className="p-5 sm:p-7">
        ...
        {searched && !loading && (
          <div className="mt-8">
            ...
            {results.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center"><p className="font-black text-slate-800">No sufficiently close matches.</p><p className="mt-2 text-sm leading-6 text-slate-500">Try the central clinical term, remove extra words, or upload a clearer lecture document. LenxiQ will not pad the list with loosely related questions.</p></div> : <div className="space-y-4">{results.map((result, index) => <ResultCard key={result.id} result={result} index={index} />)}</div>}
          </div>
        )}

        {practicalLocked && <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black text-amber-950">Practical matches are premium-only.</p><p className="mt-1 text-sm leading-6 text-amber-900/70">Upgrade to unlock specimen images and the full practical question set.</p></div><Link href="/pricing" className="rounded-xl bg-[#0B1220] px-5 py-3 text-center text-sm font-black text-white hover:bg-slate-700">View plans</Link></div>}
      </div>
    </section>
  );
}
