...
function TeachingRoom() {
  ...
  const deleteHistoryConversation = async (conversation: Conversation) => {
    if (!window.confirm(`Delete ${conversation.title}? Its saved teaching transcript will leave History.`)) return;
    const response = await fetch(`/api/teaching/conversations/${conversation.id}`, { method: 'DELETE' });
    if (!response.ok) { setError('Unable to delete this teaching history.'); return; }
    setConversations((previous) => previous.filter((item) => item.id !== conversation.id));
    if (conversationId === conversation.id) { setConversationId(null); setMessages([]); setAttachments([]); }
  ...
  const uploadAttachment = async (id: string, file: File) => {
    const prepareResponse = await fetch('/api/uploads/lecture', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scope: 'teaching', fileName: file.name, mimeType: file.type || 'application/octet-stream', sizeBytes: file.size }) });
    ...
    const { error: uploadError } = await supabase.storage.from(uploadData.bucket).uploadToSignedUrl(uploadData.path, uploadData.token, file, { contentType: uploadData.mimeType });
    if (uploadError) throw new Error('The lecture upload could not be completed. Please try again.');
    const response = await fetch('/api/teaching/attachments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ conversationId: id, storagePath: uploadData.path, fileName: file.name, mimeType: uploadData.mimeType, sizeBytes: file.size }) });
      ...
      <header className="sticky top-[4.5rem] z-30 border-b border-slate-200 bg-white/95 px-4 py-3 shadow-sm backdrop-blur sm:px-6">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          ...
          <div ref={historyContainerRef} className="relative flex shrink-0 items-center gap-2"><Link href="/pricing" className="rounded-lg border border-[#E8A23D]/50 bg-[#FFF8E9] px-3 py-2 text-xs font-black text-[#8B5709] hover:bg-[#FFF0CF]">Plans</Link><button type="button" onClick={() => setShowHistory((open) => !open)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:border-slate-400" aria-expanded={showHistory}>History{conversations.length > 0 ? ` · ${conversations.length}` : ''}</button><button type="button" onClick={handleNewSession} className="rounded-lg bg-[#0B1220] px-3 py-2 text-xs font-black text-white hover:bg-slate-800">New</button>
            {showHistory && <div className="absolute right-0 top-12 z-50 w-[min(92vw,26rem)] max-h-[min(72vh,38rem)] overflow-y-auto overscroll-contain rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between bg-white px-3 py-2"><p className="text-xs font-black uppercase tracking-[0.15em] text-slate-400">Continue a session</p><button type="button" onClick={() => setShowHistory(false)} className="text-xs font-bold text-slate-400">Close</button></div>{historyLoading && <p className="p-3 text-sm text-slate-400">Loading history…</p>}{!historyLoading && conversations.length === 0 && <p className="p-3 text-sm leading-6 text-slate-500">Your completed teaching sessions will appear here.</p>}{conversations.map((conversation) => <div key={conversation.id} className={`flex items-center gap-2 rounded-xl px-2 py-2 hover:bg-slate-50 ${conversation.id === conversationId ? 'bg-slate-100' : ''}`}><button type="button" onClick={() => void loadConversation(conversation.id)} className="min-w-0 flex-1 px-1 py-1 text-left"><span className="block truncate text-sm font-black text-slate-800">{conversation.is_pinned ? '★ ' : ''}{conversation.title}</span><span className="mt-1 block truncate text-xs font-bold text-slate-400">{conversation.course_name}</span></button><button type="button" onClick={() => void toggleHistoryPin(conversation)} className="rounded-lg px-2 py-2 text-[11px] font-black text-[#9A5D00] hover:bg-amber-50" aria-label={conversation.is_pinned ? 'Unpin teaching session' : 'Pin teaching session'}>{conversation.is_pinned ? 'Unpin' : 'Pin'}</button><button type="button" onClick={() => void deleteHistoryConversation(conversation)} className="rounded-lg px-2 py-2 text-[11px] font-black text-red-700 hover:bg-red-50" aria-label="Delete teaching session">Delete</button></div>)}</div>}
          </div>
        </div>
      </header>
