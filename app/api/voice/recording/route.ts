...
function daysRemaining(expiresAt: string | null | undefined) {
...
export async function POST(req: Request) {
  ...
  const { error: uploadError } = await supabase.storage.from('live-class-recordings').upload(storagePath, file, { contentType, upsert: false });
  if (uploadError) {
  ...
  const recordingCreatedAt = new Date();
  const recordingExpiresAt = new Date(recordingCreatedAt.getTime() + RECORDING_RETENTION_MS);
  const { error: updateError } = await supabase
    .from('teaching_conversations')
    .update({ recording_path: storagePath, recording_mime_type: contentType, recording_size_bytes: file.size, recording_created_at: recordingCreatedAt.toISOString(), recording_expires_at: recordingExpiresAt.toISOString() })
    .eq('id', conversationId)
    .eq('user_id', user.id);
  if (updateError) {
  ...
  const { data: signedUrl } = await supabase.storage.from('live-class-recordings').createSignedUrl(storagePath, 60 * 60 * 24);
  return NextResponse.json({ ok: true, recording: { mimeType: contentType, sizeBytes: file.size, signedUrl: signedUrl?.signedUrl ?? null, createdAt: recordingCreatedAt.toISOString(), expiresAt: recordingExpiresAt.toISOString(), daysRemaining: daysRemaining(recordingExpiresAt.toISOString()) } });
}
...
export async function GET(req: Request) {
  ...
  const { data: conversation } = await supabase
    .from('teaching_conversations')
    .select('recording_path, recording_mime_type, recording_size_bytes, recording_created_at, recording_expires_at')
    .eq('id', conversationId)
    .eq('user_id', user.id)
    .eq('session_type', 'live_class')
    .is('deleted_at', null)
  ...
  if (conversation.recording_expires_at && new Date(conversation.recording_expires_at).getTime() <= Date.now()) return NextResponse.json({ recording: { expired: true, expiresAt: conversation.recording_expires_at } });
  ...
  return NextResponse.json({ recording: { signedUrl: signedUrl.signedUrl, mimeType: conversation.recording_mime_type, sizeBytes: conversation.recording_size_bytes, createdAt: conversation.recording_created_at, expiresAt: conversation.recording_expires_at, daysRemaining: daysRemaining(conversation.recording_expires_at) } });
}
