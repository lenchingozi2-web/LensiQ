...
export default async function BrowseModePage({ params }: { params: Promise<{ subject: string, division: string, type: string }> }) {
  ...
  if (questionType === 'practical') {
    ...
    if (!practicalAccess.allowed) {
      ...
      return <main className="min-h-[calc(100vh-4.5rem)] bg-[#F6F8FB] px-4 py-12 text-center sm:px-6"><div className="mx-auto max-w-xl rounded-3xl border border-amber-200 bg-white p-8 shadow-xl"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#9A5D00]">Practical access</p><h1 className="mt-3 text-3xl font-black text-[#0B1220]">Choose your free practical system</h1><p className="mt-4 leading-7 text-slate-600">{practicalAccess.message}</p>{canChooseSystem && <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-left"><p className="text-xs font-black uppercase tracking-[0.15em] text-emerald-700">One free choice per account</p><p className="mt-1 text-sm font-bold leading-6 text-emerald-900">Choose one practical organ/system from the catalogue. That system becomes your single free practical access across the platform; all other practical systems require a subscription.</p></div>}<div className="mt-7 flex flex-wrap justify-center gap-3">{canChooseSystem && <Link href={selectedSystemHref} className="rounded-xl bg-emerald-600 px-6 py-3 font-black text-white hover:bg-emerald-700">{practicalAccess.needsFreePracticalSelection ? 'Choose a free system' : 'Open your free practical'}</Link>}<Link href="/pricing" className="rounded-xl bg-[#E8A23D] px-6 py-3 font-black text-[#0B1220] hover:bg-amber-500">View subscription plans</Link><Link href={isAnatomicalAggregate ? `/browse/${subjectId}` : '/browse'} className="rounded-xl border border-slate-200 px-6 py-3 font-bold text-slate-700">{isAnatomicalAggregate ? 'Back to Pathology practicals' : 'Back to practice'}</Link></div></div></main>;
    }
  }
