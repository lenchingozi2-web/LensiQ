...
function VoiceTutorContent() {
  ...
  const [soundscapeVolume, setSoundscapeVolume] = useState(0.18);
  ...
  const playSoundscape = async (selected: Soundscape) => {
    ...
    setSoundscapeStatus(`${selected.label} is playing · ${Math.round(soundscapeVolume * 100)}% device volume setting`);
  };
  ...
  const toggleDictation = () => {
    ...
    if (!Recognition) { setError('Voice typing is not supported in this browser. Speak directly to the Live Class microphone or use Chrome on Android/Safari.'); return; }
    const recognition = new Recognition();
    recognition.continuous = false;
    recognition.interimResults = true;
    recognition.lang = 'en-NG';
    const prefix = currentInput.trim();
    recognition.onresult = (event) => { let transcript = ''; for (let index = event.resultIndex; index < event.results.length; index += 1) transcript += event.results[index][0].transcript; setCurrentInput(`${prefix}${prefix && transcript ? ' ' : ''}${transcript}`.trimStart()); };
    recognition.onerror = () => { setIsDictating(false); setError('Voice typing was not available. You can still speak directly to the Live Class tutor.'); };
    recognition.onend = () => { recognitionRef.current = null; setIsDictating(false); };
    recognitionRef.current = recognition;
    setError(''); setIsDictating(true); recognition.start();
  };
  ...
  return <div className="min-h-[calc(100vh-4.5rem)] bg-[#F4F1EA] text-[#172033]">
    ...
    <audio ref={ambientAudioRef} src={ambientTrack.src} loop preload="auto" className="sr-only" aria-label={`${ambientTrack.label} learning soundscape`} />
