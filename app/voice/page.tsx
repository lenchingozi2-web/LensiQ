...
function VoiceTutorContent() {
  ...
  const toggleAmbient = async () => {
    ...
    try {
      ...
      setError('Tap the selected soundscape again to allow the optional ambience to play.');
    }
  };
  ...
  const selectSoundscape = async (id: string) => {
    ...
    try {
      ...
      setError('The soundscape is selected. Tap it again if your browser asks to start audio.');
    }
  };
  ...
  const startSession = async () => {
    ...
    try {
      ...
      if (mode === 'class' && ambientEnabled && ambientAudioRef.current) {
        ...
        void ambientAudioRef.current.play().catch(() => setError('Live Class connected. Select a soundscape if you want optional background ambience.'));
      }
